...
% Fix spacing before TOC title and between title and entries
...
% \section{Notations}
% List of things, that I used but not poperly defines \begin{itemize}
%     \item $\|x-y\|$ Norm. in different settings this can mean different things, but at the first introduction this is the 2-Norm in $R^3$, somethimes frobeniusnorm
%     \item a.e. almost everywhere
%     \item median
%     \item sign
%     \item norms L2, W12 = H1, Wkp
%     \item x-iS
%     \item as number of points in set
% \end{itemize}
\newpage
...
\begin{definition}%\textbf{(Lower semicontinuity, Coercivity and existence of Minimizers)}
...
\end{definition}
% \begin{example}\label{exampleheatEL}
% In some cases the Euler-Lagrange equation of the map $I$ corresponds to a PDE in weak formulation. As an example we consider the heat equation in $\R^n$. Let \mbox{$u: \R^+\times\R^n \rightarrow \R$} be a solution to
%     $$\partial_t u - \Delta u = 0,
%     $$
%     and $u(0, \cdot)=u_0(\cdot)$ for some given initial data $u_0: \R^n \rightarrow\R$. Lets for simplicity assume that $u, u_0$ are suficiently regular. \\
%     Then $u$ is connected to the integral functional
%     $$\mathcal{L} [v] := \frac{1}{2}\int_{\R^n}  v\partial_t v  + \|\nabla v \|^2 \,\mathrm{dx}$$ by the Euler-Lagrange equation. \\
%     Indeed, let $u_\tau := u + \tau w$ and by density we can assume that $w\in C_c^\infty$, then computing the Gateaux derivative we see:
% \begin{align*}
%      \frac{d}{d\tau}\mathcal{L} [u_\tau]|_{\tau = 0} &= \frac{1}{2}\frac{d}{d\tau}\int_{\R^n}  u_\tau\partial_t u_\tau  + \frac{1}{2}\|\nabla u_\tau \|^2 \,\mathrm{dx}|_{\tau = 0}\\
%      &=\frac{1}{2}\int_{\R^n}  w\partial_t u_\tau + u_\tau\partial_t w  + 2\nabla u \cdot \nabla w + 2\tau \|\nabla w \|^2 \,\mathrm{dx}|_{\tau = 0}\\
%      &=\frac{1}{2}\int_{\R^n}  w\partial_t u + u\partial_t w  +2\nabla u \cdot\nabla w  \,\mathrm{dx} \\
%      &=\int_{\R^n}  w\partial_t u   +\nabla u \cdot\nabla w  \,\mathrm{dx}     
%\end{align*}
%In the last step we performed integration by parts twice, using the regularity assumption from above. The resulting integral is the weak formulation of the heat equation.
%\end{example}
...
%\begin{theorem}\textbf{(Universal approximation property:)}\cite{HORNIK1989359}
%    "The universal approximation theorem for general nonlinear activation functions was rigorously established in the paper "Multilayer feedforward networks are universal approximators" by Kurt Hornik in 1989. This work demonstrated that multilayer feedforward networks with arbitrary squashing functions can approximate any Borel measurable function to any desired degree of accuracy, provided sufficient hidden units are available."
%\end{theorem}
...
$$\mathcal M_c=\{x\in\R^3 \text{ , s.t. } \psi(x)=c\}.$$
    %Let $\phi: \R^3 \rightarrow\R$ be an SDF. Then we define its $c$-level set by $$\mathcal{M}_c := \{x\in\R^{3}, \text{ s.t. } \phi (x) = c\}.$$
    %For a general function $\hat{\phi}: \R^{d+1} \rightarrow \R$ with $\nabla \phi \neq 0$ a.e. we define $$\mathcal{M}_c := \{x\in\R^{d+1}, \text{ s.t. } \phi (x) = c\}.$$
...
\]
\end{definition}
%\begin{definition}\textbf{First \& second fundamental form}
...
%Since within this thesis we want to build a framework to solve PDEs on general manifolds, we will now focus on the construction of neural SDFs, before we continue our study on neural PDEs on neural level-set surfaces in section ?.\\  
...
\end{enumerate}
% \begin{itemize}
%     \item first we solve the heat equation, $\partial_t u - \Delta u = 0$ for a small time $t$, using the points $\{x^\S_i\}$ as input; we refer to this step as \emph{heat step}.
%     \item we then compute the SDF $\phi: \mathbb{R}^3\rightarrow\R$ by requiring, first, that $|\phi|$ is small on the set $\{x_i^\mathcal{S}\}$, and second, that $\nabla \phi$, respectively $-\nabla \phi$, matches the normalized gradient of $u$ inside or outside of $\mathcal{S}$; we refer to this step as \emph{SDF step}.
% \end{itemize}
...
%, see \cref{UnorientedNormals}.\\ \begin{figure}
    %\centering
    %\includegraphics[scale=0.4]{placeholder_grafics/unoriented normals...}
    %\label{UnorientedNormals}
    %\caption{placeholder}
%\end{figure}
...
\end{proof}
\paragraph{Singular Hessian:}%In practice, we will use SIREN network architecture to approximate minimizers of the functional presented above; just by there nature of the $\sin$ activation function, this will lead to oszillations of the approximated SDF $\phi_approx$. To surpress these oscillations, we propose yet another loss, which is similar to the ideas of (ref Hessian paper). For a more detailled comparison between the results, see (ref).\\
...
For any $\epsilon_{Hess} > 0$ we have that $$ \mathcal E_{Hess}(\phi) := \int_\Omega \big|\det(D^2 \phi(x))\big| \,\mathrm{dx} + \epsilon_{Hess} \|\phi\|_{W^{2,p}}^p $$%\|D^2 \phi (x)\|_F^p \,\mathrm{dx} $$ 
...
\begin{proof}
    ...
    %(Since by assumption $\int_\Omega \|D^2 \phi_i\|^p\rightharpoonup \int_\Omega \|D^2 \phi\|^p$ )
    %We can assume that $$\int_\Omega \|D^2 \phi\|_{F}^p \leq C < \infty.$$
    ...
    %Regarding the $\det$ term we refer to \cite{Ball}, which proofs weak (seq.) continuity of $F(w) := \int_\Omega \det (\nabla w)$ in $W^{1,2}(\Omega, \R^n)$. Thus this especially implies weak continuity of the determinant of the Hessian.\\
    %Using the boundedness we conclude $\phi_i \rightarrow\phi$ in $H^1$ and $L^2$. Since $\det(\cdot)$ is weakly continuous we conclude that $$\int_\Omega \det(D^2\phi_i) \rightarrow \int_\Omega \det(D^2\phi) $$\\
    ...
    %We are left to show coercivity. Let $\phi_i$ be s.t. $\|\phi_i\|_{W^{2,p}} \rightarrow\infty$ and let $\phi = 0$ on $\S$. We have  $0 \leq \int |\det(D^2\phi)| $, thus 
    %$$\mathcal{L}_{Hess}(\phi_i) \geq \epsilon_{Hess}  \|D^2\phi_i\|_{L^p(\Omega)}^p \geq C \|\phi_i\|^p_{W^{2,p}},$$
    %where we used Poincar\'e inequality.
    
    
    %$$\mathcal{L(\phi)}_{Hess} + \lambda_2\mathcal{L(\phi)}_{SDF} +\lambda_1\mathcal{L(\phi)}_{normal} \geq  \epsilon \int\|D^2\phi\|_F^p +\lambda_2\|\nabla \phi\| ^2_{L^2(\Omega)} + \lambda_1 \|\phi\| ^2_{L^2(\S)} \rightarrow  \infty.$$ %Since the L2 norm can be bounded by 
    %Here for the $\mathcal{L(\phi)}_{SDF}$ and $\mathcal{L(\phi)}_{normal}$ term we used the bounds derived in \cref{ProofExSDF}. Since the right side defines a norm equivalent to the $W^{2,p}$-norm this concludes the proof for $\forall \epsilon > 0 $ and $p \geq 3$.
\end{proof}
...
\end{remark}
    ...
    \begin{algorithm}\label{improvedInsideOutside}\textbf{(Improved inside/outside segmentation:)}
        ...
        \end{enumerate}
    \end{algorithm} 
    %We show that this approach provides satisfying results where the box-algorithm fails in (link). 
...
% is a  improves the \emph{Sign Agnostic Learning} method \cite{SAL}, approximating the point wise unsigned distance $$f(x) := \min_{y\in\mathcal{P}} \|y - x\|$$ to an input point cloud $\mathcal{P}$ and includes a derivative loss: $$L_{SALD} = \int_\Omega \tau(f(x), \phi(x)) + \lambda \tau (\nabla f(x), \nabla \phi(x)) \,\mathrm{dx}, \quad \tau(a,b) := \min(\|a - b\|, \|a + b\|)$$
% They recover an signed distance from unoriented data relying on specific initialization, namely they initialize with an approximation of the unit sphere. 
...
During training, the individual scaling factors in front of the integrals get changed, depending on a complex routine established by the authors.\\ While being similar to our approach due to its connection to the heat equation, the method still depends on the Eikonal equation, making it non-convex and not well-posed.% Furthermore they don't take the density of the point cloud into account, leading to problems with nonuniform point clouds. 
%With this overview in mind, the NeatSDF method can be viewn as a mixture of the SALD, Hessian and HotSpot method: We use the relation between heat equation and distance to approximate unoriented normals; we then orient the normals similarly to SALD using the $\min(\cdot, \cdot)$. The regularization of the Hessian term across the interface ensures the correct orientation an suppresses oscillation. 
...
% \begin{figure}
%     \centering
%         \includegraphics[scale=0.1]{placeholder_grafics/hand_rendering2.png}
%         \includegraphics[scale=0.15]{placeholder_grafics/pipes.png}
%     \caption{Test figure}
%     \label{heat_noise_results}
    
% \end{figure}
% \begin{figure}
%     \centering
%     \includegraphics[width=\linewidth]{Figures/Hand_Comparison.pdf}
%     \caption{Qualitative evaluation on a hand mesh.}
...
First we consider different network architecture and batch sizes, \cref{poisson_diff_bs}. Here the boundary condition $\int_\S u\ \mathrm{da} = 0$ is imposed after training by shifting the function values by the surface mean value. \par %Additionally we compare results using the grund truth function $\phi_{gt}$.\\
 \begin{figure}
    \centering
...
%We demonstrate that the exact imposition of Dirichlet boundary conditions can, in some cases, improve the learning of neural signed distance functions (SDFs). Consider the ground truth SDF, denoted as $\phi_{gt}$, which is zero on the surface and positive elsewhere. We incorporate this property by using $\phi_{gt}$ in the final layer of the network to explicitly enforce the zero-level set during training. While not providing any major improvements for the SDF quality (besides zero-level set reconstruction) the results indicate a significant overall improvement in performance comparing the approximations of the poisson problem (see \cref{poissonpb}).
\begin{figure}[t]
    \centering
    \begin{minipage}[c]{0.45\linewidth}
        ...
        \begin{tabular}{lcc}
            ...
            \midrule
            %$\phi_{gt}$, $\psi = 1$       & 0.0134849325   \\
            %$\phi_{gt}$, $\psi = x_1$       & 0.009344  \\
            HeatSDF &$1$     & 0.00589 \\
            HeatSDF & $|x_1|$     &  0.00715 \\
            NeatSDF& $ 1$     & 0.00583   \\
            NeatSDF& $|x_1|$     & 0.00421   \\
...
\end{figure}
    ...
    %\begin{tabular}{ccc}
    %     \toprule
    %     \textbf{Input SDF method} & mean error & max error\\
    %     NeatSDF \\
    %     NeatSDF + boundary\\
    %       \midrule
    %     \\ 
    ...
    % \begin{tabular}{ccc}
    %     \toprule
    %     \textbf{Input SDF method} & Eikonal ...\\
    %     NeatSDF \\
    %     NeatSDF + boundary\\
    %       \midrule
    %     \\ 
...
%         \bottomrule
%     \end{tabular}
% \caption{Comparison of different parameter settings to FEM solution}
%     \label{Heatflow1}
% \end{table}
% \begin{table}[]
%     \begin{tabular}{cc}
...
%         \bottomrule
%     \end{tabular}
% \caption{Evaluation of different time steps for point initial data}
%     \label{Heatflow1}
% \end{table}
\begin{figure}
...
%As discussed above, one could do that by adding an additional loss term $$\mathcal{L}_{Poisson-Bd.} := \lambda_{bd} \int_{\{u_0 \neq 0\}} 1 da.$$
...
%For discrete surfaces (mesh or point clouds) \cite{Crane_2013} uses $t = h^2$ as the time parameter in the first step for $h$ being the mean spacing between adjacent nodes of the mesh.\\
%However, this doesn't transfer to the neural setting. We computed geodesic distances using several time step values and found that \( t = \text{?} \) yields the best trade-off between accuracy near the source point (where the distance is zero) and accuracy farther away. For a detailed comparison, see \cref{fig:geodesic_time_comparison}. \\
%For point clouds we therefore propose to use $$\hat{h} = \frac{1}{Nk} \sum_{i\leq N} \sum_{j\leq k} \min_{i\neq j} d(p_i, p_{j(i)})$$
%where $N$ is the number of points $p_i$ and $p_j(i)$ are the $k$-nearest neighbors of $p_i$, $k \in \mathbb{N}$.\\
%\subsubsection{Eigenproblem}
%Using the same network architecture as described above, we do now turn our attention towards solving eigenvalue problems, namely solving:
%\begin{align}
%    \Delta u + \lambda u = 0\label{strongEVP}
%\end{align}
%which can be realized in a neural network setting either via a PINN setting by minimizing the squared residuum, or alternativly by minimizing the energy 
%\begin{align}
%    \mathcal{E}[\psi] := \frac{1}{2}\int |\nabla \psi|^2 + \lambda |\phi|^2 \label{eigenproblem}
%\end{align}
%where by the Euler-Lagrange equation a solution of \eqref{eigenproblem} corresponds to the weak formulation of \eqref{strongEVP}
